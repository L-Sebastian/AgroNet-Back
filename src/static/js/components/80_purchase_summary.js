document.addEventListener("DOMContentLoaded", () => {
  ...
  if (container) {
    fetch("/frontend/public/views/components/80_purchase_summary.html")
      .then(res => res.text())
      .then(html => {
  ...
  function initPopups() {
    ...
    closeSuccess.addEventListener("click", () => {
      ...
      window.location.href = "/frontend/public/views/customer-pages/historial_orders.html"; //  Redirigir al cerrar
    });

    successPopup.addEventListener("click", e => {
      if (e.target === successPopup) {
        successPopup.classList.remove("show");
        window.location.href = "/frontend/public/views/customer-pages/historial_orders.html";
      }
    });
